...
class HouseCalendar {
  ...
  generateWeekRates(monthKey, weekReferenceDate, visibleRateColumns) {
    ...
    const fridayMonthKey = weekFriday.getFullYear() + '-' + String(weekFriday.getMonth() + 1).padStart(2, '0');

    // Get Friday's date as the week key - API data is always keyed by Friday
    const fridayDateKey = weekFriday.getFullYear() + '-' + String(weekFriday.getMonth() + 1).padStart(2, '0') + '-' + String(weekFriday.getDate()).padStart(2, '0');
    for (const rateKey of visibleRateColumns) {
      ...
      const checkinDate = this.getCheckinDateForRate(weekReferenceDate, rateKey, monthKey);
      const checkinDateKey = checkinDate.getFullYear() + '-' + String(checkinDate.getMonth() + 1).padStart(2, '0') + '-' + String(checkinDate.getDate()).padStart(2, '0');
  ...
  getCheckinDateForRate(weekReferenceDate, rateKey, monthKey) {
    ...
    const fridayMonthKey = friday.getFullYear() + '-' + String(friday.getMonth() + 1).padStart(2, '0');
    switch (rateKey) {
      ...
      case '90':
        {
          // 5 nights - no fixed checkin day, any day in the week works
          const candidates90 = [];
          for (let i = 0; i < 7; i++) {
          ...
          return this.findBestCheckinDate(candidates90, rateKey, fridayMonthKey, monthKey);
        }
      default:
        return friday;
    }
  ...
  isStayAvailable(checkinDate, nights) {
    for (let i = 0; i < nights; i++) {
      ...
      const dayData = this.calendarData.availability?.[this.formatDateLocal(night)];
    ...
    for (const date of possibleDates) {
      ...
      if (dateMonth === calendarMonthKey) {
        ...
        if (weekRates && weekRates[rateKey]) {
          ...
          if (rateData.type && !['unavailable', 'hidden'].includes(rateData.type) && this.isStayAvailable(date, nights)) {
            return date;
          }
        }
      }
  ...
  getWeekRate(calendarMonthKey, storageMonthKey, weekDateKey, rateKey, checkinDateKey) {
    ...
    if (!monthRates.weeks[weekDateKey]) {
      ...
      const previousWeekDate = this.findPreviousWeekDate(storageMonthKey, weekDateKey);
    ...
    if (weekRates && weekRates[rateKey]) {
      ...
      if (rateData.type === 'previous') {
        return this.findPreviousWeekRate(storageMonthKey, weekDateKey, rateKey);
      }
      return rateData;
    }
