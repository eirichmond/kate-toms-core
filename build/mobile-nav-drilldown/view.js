...
/***/ "./blocks/mobile-nav-drilldown/view.js":
/*!*********************************************!*\
  !*** ./blocks/mobile-nav-drilldown/view.js ***!
  \*********************************************/
...
/* harmony import */ var _style_css__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./style.css */ "./blocks/mobile-nav-drilldown/style.css");
/**
 ...
 */
const MODULE_DATA_ID = 'wp-script-module-data-kate-toms-core/mobile-nav-drilldown';
...
document.addEventListener('focusout', event => {
  ...
  if (!overlay || !overlay.classList.contains(OVERLAY_OPEN_CLASS)) {
    return;
  }
...
function buildChildPanel(parentLi) {
  ...
  const sourceList = Array.from(parentLi.children).find(child => child.tagName === 'UL');
  if (!sourceList) {
    return null;
  }
  ...
  const sourceLevel = Number.parseInt(sourcePanel.getAttribute('data-level') || '0', 10);
  ...
  const backLabel = sourceLevel === 0 ? 'Menu' : sourcePanel.dataset.parentLabel || 'Menu';
...
function applyWrapperTransform(wrapper) {
  ...
  const panels = Array.from(track.querySelectorAll(`:scope > .${PANEL_CLASS}`));
  ...
  const activeId = newLength === 0 ? null : state.drilldownPath[newLength - 1];
  panels.forEach(panel => {
    const isActive = activeId === null ? panel.dataset.level === '0' : panel.id === activeId;
    if (isActive) {
...
function syncChevronExpanded(track) {
  const triggerLinks = track.querySelectorAll('li[data-drilldown-parent="true"] > a[aria-expanded]');
  triggerLinks.forEach(link => {
    const li = link.parentElement;
    const panel = li ? childPanelCache.get(li) : null;
    const expanded = !!(panel && state.drilldownPath.includes(panel.id));
...
function drillBack(track) {
  ...
  if (newActive) {
    const target = newActive.querySelector(`.${BACK_BUTTON_CLASS}`) || newActive.querySelector('a, button');
    if (target) {
      target.focus({
        preventScroll: true
      });
...
function injectChevronsInList(list, arrowSrc) {
  const parents = list.querySelectorAll(':scope > li[data-drilldown-parent="true"]');
  parents.forEach(li => {
...
function onOverlayOpen(overlay) {
  ...
  wrapOverlay(overlay);
  const rootList = overlay.querySelector(`.${PANEL_CLASS}[data-level="0"] ${ROOT_LIST_SELECTOR}`);
  if (rootList) {
...
const CONTACT_FORM_TRIGGER_SELECTOR = '.wp-block-kate-and-toms-icon-button[data-show-form="true"][data-form-type="contact"]';
document.addEventListener('click', event => {
  ...
  const openOverlay = document.querySelector(`${OVERLAY_SELECTOR}.${OVERLAY_OPEN_CLASS}`);
  if (openOverlay) {
    const closeBtn = openOverlay.querySelector('.wp-block-navigation__responsive-container-close');
    if (closeBtn) {
      closeBtn.click();
    }
...
function ensureSearchOverlay() {
  let overlay = document.querySelector(`.${MOBILE_SEARCH_OVERLAY_CLASS}`);
  if (overlay) {
    return overlay;
  }
...
function closeMobileSearch() {
  ...
  if (searchBlock && searchOriginalParent) {
    if (searchOriginalNextSibling) {
      searchOriginalParent.insertBefore(searchBlock, searchOriginalNextSibling);
    } else {
...
document.addEventListener('keydown', event => {
  if (event.key === 'Escape' && document.body.classList.contains(MOBILE_SEARCH_OPEN_CLASS)) {
    closeMobileSearch();
  }
});
...
/***/ "./blocks/mobile-nav-drilldown/style.css":
/*!***********************************************!*\
  !*** ./blocks/mobile-nav-drilldown/style.css ***!
  \***********************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/******/ 	var installedChunks = {
/******/ 		"mobile-nav-drilldown/view": 0,
/******/ 		"mobile-nav-drilldown/style-view": 0
/******/ 	};
/******/ 	
/******/ 	// no install chunk
/******/ 	
...
/******/ var __webpack_exports__ = __webpack_require__.O(undefined, ["mobile-nav-drilldown/style-view"], () => (__webpack_require__("./blocks/mobile-nav-drilldown/view.js")))
