...
async function loadMoreFor(region, context) {
  ...
  try {
    ...
    if (regionContext.contextTaxonomy && regionContext.contextTerm) {
      ...
      params.append('context_term', regionContext.contextTerm.toString());
    }
    ...
    async refreshResults(event) {
      ...
      try {
        ...
        if (context.contextTaxonomy && context.contextTerm) {
          params.append('context_taxonomy', context.contextTaxonomy);
          params.append('context_term', context.contextTerm.toString());
        }
        const apiUrl = `/wp-json/kate-toms/v1/houses?${params.toString()}`;
        const response = await fetch(apiUrl);
        if (!response.ok) {
