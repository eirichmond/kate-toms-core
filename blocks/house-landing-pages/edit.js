...
export default function Edit( { attributes, setAttributes } ) {
	...
	const { taxonomyTerms } = useSelect( ( select ) => {
		...
		return {
			taxonomyTerms: {
				location:
					getEntityRecords( 'taxonomy', 'location', {
						per_page: -1,
					} ) || [],
				size:
					getEntityRecords( 'taxonomy', 'size', { per_page: -1 } ) ||
					[],
				type:
					getEntityRecords( 'taxonomy', 'type', { per_page: -1 } ) ||
					[],
				occasion:
					getEntityRecords( 'taxonomy', 'occasion', {
						per_page: -1,
					} ) || [],
				feature:
					getEntityRecords( 'taxonomy', 'feature', {
						per_page: -1,
					} ) || [],
			},
		};
	...
	const { houses, hasResolved } = useSelect(
		( select ) => {
			const query = {
			...
			if ( locationConfig.slug && taxonomyTerms.location ) {
				const locationTerm = taxonomyTerms.location.find(
					( term ) => term.slug === locationConfig.slug
				);
				if ( locationTerm ) {
			...
			Object.keys( taxonomyFilters ).forEach( ( taxonomy ) => {
				if (
					...
					taxonomyFilters[ taxonomy ].length > 0 &&
					taxonomyTerms[ taxonomy ]
				) {
					const termIds = taxonomyFilters[ taxonomy ]
						.map( ( slug ) => {
							const term = taxonomyTerms[ taxonomy ].find(
								( t ) => t.slug === slug
							);
	...
	return (
		<>
			<InspectorControls>
				...
				<PanelBody
					title={ __( 'Additional Filters', 'kate-toms-core' ) }
				>
					<FormTokenField
						label={ __( 'Location', 'kate-toms-core' ) }
						value={ ( taxonomyFilters.location || [] ).map(
							( slug ) => {
								const term = taxonomyTerms.location.find(
									( t ) => t.slug === slug
								);
								return term ? term.name : slug;
							}
						) }
						suggestions={ ( taxonomyTerms.location || [] ).map(
							( term ) => term.name
						) }
						onChange={ ( tokens ) => {
							const slugs = tokens.map( ( token ) => {
								const term = (
									taxonomyTerms.location || []
								).find( ( t ) => t.name === token );
					...
					<FormTokenField
						...
						value={ ( taxonomyFilters.size || [] ).map(
							( slug ) => {
								const term = taxonomyTerms.size.find(
									( t ) => t.slug === slug
								);
								return term ? term.name : slug;
							}
						) }
						suggestions={ ( taxonomyTerms.size || [] ).map(
							( term ) => term.name
						) }
						onChange={ ( tokens ) => {
							const slugs = tokens.map( ( token ) => {
					...
					<FormTokenField
						...
						value={ ( taxonomyFilters.type || [] ).map(
							( slug ) => {
								const term = taxonomyTerms.type.find(
									( t ) => t.slug === slug
								);
								return term ? term.name : slug;
							}
						) }
						suggestions={ ( taxonomyTerms.type || [] ).map(
							( term ) => term.name
						) }
						onChange={ ( tokens ) => {
							const slugs = tokens.map( ( token ) => {
					...
					<FormTokenField
						...
						value={ ( taxonomyFilters.occasion || [] ).map(
							( slug ) => {
								const term = taxonomyTerms.occasion.find(
									( t ) => t.slug === slug
								);
								return term ? term.name : slug;
							}
						) }
						suggestions={ ( taxonomyTerms.occasion || [] ).map(
							( term ) => term.name
						) }
						onChange={ ( tokens ) => {
							const slugs = tokens.map( ( token ) => {
								const term = (
									taxonomyTerms.occasion || []
								).find( ( t ) => t.name === token );
								return term ? term.slug : token;
							} );
							setAttributes( {
					...
					<FormTokenField
						...
						value={ ( taxonomyFilters.feature || [] ).map(
							( slug ) => {
								const term = taxonomyTerms.feature.find(
									( t ) => t.slug === slug
								);
								return term ? term.name : slug;
							}
						) }
						suggestions={ ( taxonomyTerms.feature || [] ).map(
							( term ) => term.name
						) }
						onChange={ ( tokens ) => {
							const slugs = tokens.map( ( token ) => {
								const term = (
									taxonomyTerms.feature || []
								).find( ( t ) => t.name === token );
								return term ? term.slug : token;
							} );
							setAttributes( {
			...
			<div { ...blockProps }>
				<div className="house-landing-pages-preview">
					<h3>
						{ __( 'House Landing Pages Block', 'kate-toms-core' ) }
					</h3>
					<p>
						{ __( 'Location:', 'kate-toms-core' ) }{ ' ' }
						<strong>{ locationConfig.name }</strong>
						<br />
						{ __( 'Ordering by:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ orderBy === 'meta_value_num'
								? 'Sleeps (Meta Value)'
								: orderBy }
					...
					{ ! hasResolved ? (
						...
						<div className="houses-preview">
							<p>
								<strong>
									{ __( 'Found:', 'kate-toms-core' ) }{ ' ' }
									{ houses.length === 100 &&
									postsPerPage === -1
										? '100+'
										: houses.length }{ ' ' }
									{ __( 'houses', 'kate-toms-core' ) }
								</strong>
							</p>
							<p>
								<em>
									{ __(
										'Note: Editor preview shows REST API results, which may differ slightly from frontend due to query limitations.',
										'kate-toms-core'
									) }
								</em>
							</p>
