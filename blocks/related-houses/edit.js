...
export default function Edit( { attributes, setAttributes } ) {
	const { house1Id, house2Id, house3Id, house4Id, saveToSubPages } =
		attributes;
	...
	const loadHousesOptions = async () => {
		...
		try {
			...
			const options = [
				{
					label: __( 'Select a house…', 'kate-toms-core' ),
					value: 0,
				},
				...houses.map( ( house ) => ( {
	...
	const loadMissingHouseTitles = async () => {
		const selectedIds = [ house1Id, house2Id, house3Id, house4Id ].filter(
			( id ) => id > 0 && ! selectedHouseTitles[ id ]
		);
		...
		try {
			const houses = await apiFetch( {
				path: `/wp/v2/houses?include=${ selectedIds.join(
					','
				) }&_fields=id,title&parent=0`,
			} );
	...
	const handleSaveToSubPages = async () => {
		if ( ! currentPost?.id ) {
			setSaveStatus( {
				type: 'error',
				message: __(
					'Unable to determine current post',
					'kate-toms-core'
				),
			} );
			return;
		...
		try {
			...
			if ( response.success ) {
				setSaveStatus( {
					type: 'success',
					message: __(
						`Successfully updated ${ response.updated_count } sub pages`,
						'kate-toms-core'
					),
				} );
			} else {
				setSaveStatus( {
					...
					message:
						response.message ||
						__( 'Failed to save to sub pages', 'kate-toms-core' ),
				} );
			}
		} catch ( error ) {
			setSaveStatus( {
				type: 'error',
				message:
					__( 'Error saving to sub pages:', 'kate-toms-core' ) +
					error.message,
			} );
		} finally {
			setIsSaving( false );
			...
			<BlockControls>
				{ saveToSubPages && currentPost?.post_type === 'houses' && (
					<ToolbarGroup>
						...
						>
							{ isSaving
								? __( 'Saving…', 'kate-toms-core' )
								: __( 'Save to Sub Pages', 'kate-toms-core' ) }
						</Button>
					</ToolbarGroup>
				) }
			</BlockControls>
			...
			<InspectorControls>
				<PanelBody title={ __( 'House Selection', 'kate-toms-core' ) }>
					<SelectControl
						...
						onChange={ ( value ) =>
							setAttributes( { house1Id: parseInt( value ) } )
						}
					/>
					<SelectControl
						...
						onFocus={ loadHousesOptions }
						onChange={ ( value ) =>
							setAttributes( { house2Id: parseInt( value ) } )
						}
					/>
					<SelectControl
						...
						onFocus={ loadHousesOptions }
						onChange={ ( value ) =>
							setAttributes( { house3Id: parseInt( value ) } )
						}
					/>
					<SelectControl
						...
						onFocus={ loadHousesOptions }
						onChange={ ( value ) =>
							setAttributes( { house4Id: parseInt( value ) } )
						}
					/>
				</PanelBody>
				...
				{ currentPost?.post_type === 'houses' && (
					<PanelBody title={ __( 'Sub Pages', 'kate-toms-core' ) }>
						<ToggleControl
							...
							help={ __(
								'When enabled, this block will be saved to all sub pages of this house, replacing any existing related houses blocks.',
								'kate-toms-core'
							) }
							checked={ saveToSubPages }
							onChange={ ( value ) =>
								setAttributes( { saveToSubPages: value } )
							}
						/>
					...
					{ saveToSubPages && currentPost?.post_type === 'houses' && (
						...
						>
							<Notice status="info" isDismissible={ false }>
								{ __(
									'This block will be saved to all sub pages of this house.',
									'kate-toms-core'
								) }
							</Notice>
						</div>
