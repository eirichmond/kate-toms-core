...
const calendarTableIcon = (
	...
	>
		...
		<rect x="3" y="9" width="2" height="2" fill="#c5c951" rx="0.5" />{ ' ' }
		{ /* Available */ }
		<rect x="7" y="9" width="2" height="2" fill="#e3634b" rx="0.5" />{ ' ' }
		{ /* Booked */ }
		<rect
			x="11"
			y="9"
...
export default function Edit( { attributes, setAttributes } ) {
	const { houseId, showRates, monthsToShow, autoRefresh, refreshInterval } =
		attributes;
	...
	return (
		<>
			<InspectorControls>
				<PanelBody
					title={ __( 'Calendar Settings', 'kate-toms-core' ) }
					initialOpen={ true }
				>
					<TextControl
						...
						help={ __(
							'Enter the house ID from your booking system',
							'kate-toms-core'
						) }
					/>
					...
					<RangeControl
						...
						value={ monthsToShow }
						onChange={ ( value ) =>
							setAttributes( { monthsToShow: value } )
						}
						min={ 1 }
						max={ 24 }
					/>
					...
					<ToggleControl
						...
						checked={ showRates }
						onChange={ ( value ) =>
							setAttributes( { showRates: value } )
						}
					/>
				</PanelBody>

				<PanelBody
					title={ __( 'Auto Refresh', 'kate-toms-core' ) }
					initialOpen={ false }
				>
					<ToggleControl
						...
						{ houseId ? (
							<>
								...
								<p>
									...
									{ showRates
										? __( 'Yes', 'kate-toms-core' )
										: __( 'No', 'kate-toms-core' ) }
								</p>
								{ autoRefresh && (
									<p>
										<strong>
										...
										{ __( 'Every', 'kate-toms-core' ) }{ ' ' }
										{ refreshInterval }{ ' ' }
										{ __( 'minutes', 'kate-toms-core' ) }
									</p>
								) }
							</>
						) : (
							<p className="calendar-placeholder">
								{ __(
									'Please configure the House ID in the block settings to display the calendar.',
									'kate-toms-core'
								) }
							</p>
						) }
