...
export default function Edit( { attributes, setAttributes } ) {
	...
	const { taxonomyTerms, isLoading } = useSelect( ( select ) => {
		...
		return {
			taxonomyTerms: {
				location: getEntityRecords(
					'taxonomy',
					'location',
					defaultQuery
				),
				feature: getEntityRecords(
					'taxonomy',
					'feature',
					defaultQuery
				),
			...
			isLoading: {
				location: isResolving( 'getEntityRecords', [
					'taxonomy',
					'location',
					defaultQuery,
				] ),
				feature: isResolving( 'getEntityRecords', [
					'taxonomy',
					'feature',
					defaultQuery,
				] ),
				size: isResolving( 'getEntityRecords', [
					'taxonomy',
					'size',
					defaultQuery,
				] ),
				type: isResolving( 'getEntityRecords', [
					'taxonomy',
					'type',
					defaultQuery,
				] ),
				occasion: isResolving( 'getEntityRecords', [
					'taxonomy',
					'occasion',
					defaultQuery,
				] ),
	...
	const featureProps = buildTokenFieldProps(
		'feature',
		featureTermIds,
		'featureTermIds'
	);
	const sizeProps = buildTokenFieldProps(
		'size',
		sizeTermIds,
		'sizeTermIds'
	);
	const typeProps = buildTokenFieldProps(
		'type',
		typeTermIds,
		'typeTermIds'
	);
	const occasionProps = buildTokenFieldProps(
		'occasion',
		occasionTermIds,
		'occasionTermIds'
	);
	...
	const formatDisplay = ( names ) =>
		names.length > 0 ? names.join( ', ' ) : __( 'None', 'kate-toms-core' );
		...
		<>
			...
			<div { ...blockProps }>
				<div className="house-load-search-preview">
					<h3>
						{ __( 'House Load Search Block', 'kate-toms-core' ) }
					</h3>
					<p>
						{ __( 'Houses per load:', 'kate-toms-core' ) }{ ' ' }
						<strong>{ postsPerPage }</strong>
						<br />
						{ __( 'Ordering:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ __(
								'Sleeps Max (Descending)',
								'kate-toms-core'
							) }
						</strong>
						<br />
						{ __( 'Locations:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ formatDisplay( locationProps.selectedNames ) }
						</strong>
						<br />
						{ __( 'Features:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ formatDisplay( featureProps.selectedNames ) }
						</strong>
						<br />
						{ __( 'Sizes:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ formatDisplay( sizeProps.selectedNames ) }
						</strong>
						<br />
						{ __( 'Types:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ formatDisplay( typeProps.selectedNames ) }
						</strong>
						<br />
						{ __( 'Occasions:', 'kate-toms-core' ) }{ ' ' }
						<strong>
							{ formatDisplay( occasionProps.selectedNames ) }
						</strong>
					</p>
					<p>
						<em>
							{ __(
								'Houses will load with infinite scroll on the frontend.',
								'kate-toms-core'
							) }
						</em>
					</p>
