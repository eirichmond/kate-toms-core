...
export default function Edit( { attributes, setAttributes } ) {
	const { date, dtype, size, local, feature, displayStyle } = attributes;
	...
	const sizeOptions = [
		{
			label: __( '2–10 People', 'kate-and-toms-houses-filter-search' ),
			value: '2',
		},
		{
			label: __( '10–20 People', 'kate-and-toms-houses-filter-search' ),
			value: '10',
		},
		{
			label: __( '20+ People', 'kate-and-toms-houses-filter-search' ),
			value: '20',
		},
	];
		...
		<div { ...blockProps }>
			<InspectorControls>
				<PanelBody
					title={ __(
						'Display Settings',
						'kate-and-toms-houses-filter-search'
					) }
				>
					<ToggleControl
						label={ __(
						...
						<ButtonGroup>
							{ durationOptions.map( ( option ) => (
								<Button
									...
									onClick={ () =>
										setAttributes( { dtype: option.value } )
									}
								>
									{ option.label }
								</Button>
						...
						<ButtonGroup>
							{ locations.map( ( option ) => (
								<Button
									...
									onClick={ () =>
										setAttributes( { local: option.value } )
									}
								>
									{ option.label }
								</Button>
