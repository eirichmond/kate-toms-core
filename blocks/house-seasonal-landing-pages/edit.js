...
export default function Edit({ attributes, setAttributes, context }) {
    ...
    const { seasonalMeta, postType } = useSelect((select) => {
        if (!postId) return { seasonalMeta: {}, postType: null };
        ...
        const post = select(coreStore).getEditedEntityRecord('postType', 'seasonal', postId);
        ...
        <>
            ...
            <div {...blockProps}>
                <div className="house-seasonal-landing-pages-preview">
                    <h3>{__('House Seasonal Landing Pages Block', 'kate-toms-core')}</h3>

                    {postType !== 'seasonal' ? (
                        <div style={{ padding: '20px', background: '#fff3cd', border: '1px solid #ffc107', borderRadius: '4px' }}>
                            <p><strong>{__('⚠️ This block only works on Seasonal post types', 'kate-toms-core')}</strong></p>
                            <p>{__('Please add this block to a seasonal post to see houses filtered by availability.', 'kate-toms-core')}</p>
                        </div>
                    ) : (
                        <>
                            <div style={{ padding: '15px', background: '#f0f0f1', borderRadius: '4px', marginBottom: '10px' }}>
                                <p><strong>{__('Seasonal Criteria:', 'kate-toms-core')}</strong></p>
                                <p>
                                    {__('Date Range:', 'kate-toms-core')} <strong>{seasonalMeta.beginning || 'Not set'}</strong> → <strong>{seasonalMeta.ending || 'Not set'}</strong><br />
                                    {__('Periods:', 'kate-toms-core')} <strong>{formatPeriods(seasonalMeta.periods_to_include)}</strong>
                                </p>
                            </div>
