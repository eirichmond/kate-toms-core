...
export default function Edit({ attributes, setAttributes }) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('House Settings', 'kate-toms-core')}>
					<SelectControl
						...
						onChange={(value) => setAttributes({ selectedHouse: parseInt(value) })}
					/>
					<SelectControl
			...
			<div {...blockProps}>
				<div className="wp-block-group has-white-background-color has-background" style={{ minHeight: '365px' }}>
					{selectedHouseData ? (
						<>
							{/* Featured Image Placeholder */}
							<div style={{ height: '200px', backgroundColor: '#f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
								<p>{__('Featured Image', 'kate-toms-core')}</p>
							</div>
							...
							{/* Description */}
							<div style={{ padding: 'var(--wp--preset--spacing--30)' }}>
								<p style={{ fontSize: 'var(--wp--preset--font-size--x-small)' }}>
									{selectedHouseData.meta?.brief_description || __('No description available', 'kate-toms-core')}
								</p>
							</div>

							{/* Footer */}
							<div style={{ padding: 'var(--wp--preset--spacing--30)', borderTop: '1px solid var(--wp--preset--color--tertiary)' }}>
								<div style={{ display: 'flex', justifyContent: 'space-between' }}>
									<div style={{ display: 'flex', gap: '0.2em' }}>
										<span style={{ fontSize: 'var(--wp--preset--font-size--x-small)' }}>
											{__('Sleeps', 'kate-toms-core')} {selectedHouseData.meta?.sleeps_min || 0} to {selectedHouseData.meta?.sleeps_max || 0}
										</span>
									</div>
									<div>
										<span style={{ fontSize: 'var(--wp--preset--font-size--x-small)' }}>
											{selectedHouseData.meta?.location_text || ''}
										</span>
									</div>
								</div>
							</div>
						...
						<div style={{ padding: '20px', textAlign: 'center' }}>
							<p>{selectedHouse ? __('Loading house data...', 'kate-toms-core') : __('Please select a house from the sidebar', 'kate-toms-core')}</p>
						</div>
					)}
				</div>
			</div>
